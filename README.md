...
It is recommended to look at the example scenes (example_scenes/) before creating your own scenes. The **example scenes will not directly work** as the plugin does not kno<style>H1{color:Blue;}</style>w where SOFA and the other applications are installed. The correct paths can be set in the root node of the project. See below:
...
* Sofa Binary Path: The path to the SOFA binary (usually called *runSofa*)
* Gmsh Binary Path: The path to the gmsh binary
* Ctmconv path: The path to the ctmconv binary
